...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Panel del Barbero</h1>
    ...
    @if(session('success'))
        <div class="bg-green-50 border border-green-200 text-green-700 text-sm px-4 py-3 rounded-lg mb-6 flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            <div class="text-3xl font-extrabold text-gray-800">{{ $totalCitas }}</div>
            <div class="text-xs text-gray-500 mt-1">{{ $citasPendientes }} pendientes / {{ $citasCompletadas }} completadas</div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            <div class="text-3xl font-extrabold text-gray-800">{{ $citasCompletadas }}</div>
            <div class="text-xs text-gray-500 mt-1">Servicios finalizados con éxito</div>
        </div>

        {{-- Tarjeta 3: Comisión Ganada (40%) --}}
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            <div class="flex items-center justify-between mb-2">
                <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Mi Comisión (40%)</span>
            ...
            <div class="text-3xl font-extrabold text-amber-600">${{ number_format($gananciaBarbero, 2) }}</div>
            <div class="text-xs text-gray-500 mt-1">Total facturado: ${{ number_format($totalFacturado, 2) }}</div>
        </div>

        {{-- Tarjeta 4: Propinas --}}
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            <div class="flex items-center justify-between mb-2">
                <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Propinas</span>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        ...
        @if($appointments->isEmpty())
            <div class="p-12 text-center text-gray-500">
                ...
                <p class="text-base font-semibold text-gray-600">No hay citas programadas para el filtro seleccionado</p>
                <p class="text-sm text-gray-400 mt-1">Intenta seleccionar otra fecha o ajustar los filtros superiores.</p>
            </div>
        @else
                    ...
                    <tbody class="divide-y divide-gray-200 text-sm">
                        @foreach($appointments as $appointment)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4">
                                    <div class="font-medium text-gray-900">{{ $appointment->client?->name ?? 'Cliente General' }}</div>
                                ...
                                <td class="px-6 py-4">
                                    <div class="font-medium text-gray-800">{{ $appointment->service?->name ?? 'Corte general' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="font-bold text-gray-900">${{ number_format($appointment->service?->price ?? 0, 2) }}</div>
                                    <div class="text-xs text-gray-500">
                                        {{ $appointment->payment_method ? $appointment->payment_method : 'No especificado' }}
                                        @if($appointment->tip > 0)
                                            <span class="text-purple-600 font-semibold">(+${{ number_format($appointment->tip, 2) }} propina)</span>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @switch($appointment->status)
                                        ...
                                        @case('confirmed')
                                            ...
                                            @break
                                        @case('in_process')
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 border border-indigo-200 animate-pulse">
                                                En Proceso (Check-in)
                                            </span>
                                            @break
                                        @case('completed')
                                    ...
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        @if($appointment->status !== 'in_process' && $appointment->status !== 'completed' && $appointment->status !== 'cancelled')
                                            <form method="POST" action="{{ route('barber.appointments.updateStatus', $appointment->id) }}" class="inline">
                                                ...
                                                <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition shadow-sm">
                                                    Check-in
                                                </button>
                                            </form>
                                        @endif
                                        ...
                                        @if($appointment->status !== 'completed' && $appointment->status !== 'cancelled')
                                            <form method="POST" action="{{ route('barber.appointments.updateStatus', $appointment->id) }}" class="inline">
                                                ...
                                                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition shadow-sm">
                                                    Completar
                                                </button>
                                            </form>
                                        @endif
                                        ...
                                        <button onclick="abrirModalEstado({{ $appointment->id }}, '{{ $appointment->status }}', '{{ $appointment->payment_method }}', '{{ $appointment->tip }}')"
                                            class="border border-gray-300 text-gray-700 hover:bg-gray-100 text-xs font-medium px-2.5 py-1.5 rounded-lg transition">
                                            Opciones
                                        </button>
                                    </div>
                                </td>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        ...
        @if($clientesRecientes->isEmpty())
            <p class="text-gray-400 text-sm py-4 text-center">Aún no hay registro de clientes completados para este barbero.</p>
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                @foreach($clientesRecientes as $citaRealizada)
                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 flex flex-col justify-between">
                        <div>
                            <div class="font-bold text-gray-800 text-sm mb-1">{{ $citaRealizada->client?->name ?? 'Cliente' }}</div>
                            <div class="text-xs text-amber-700 font-semibold mb-2">{{ $citaRealizada->service?->name }}</div>
                        </div>
                        <div class="text-xs text-gray-400 pt-2 border-t border-gray-200 flex justify-between">
                            ...
                            <span class="font-bold text-gray-700">${{ number_format($citaRealizada->service?->price ?? 0, 2) }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
...
<div id="modal-estado" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center hidden">
    <div class="bg-white rounded-xl max-w-md w-full p-6 shadow-xl relative">
        ...
        <form id="form-modal-estado" method="POST" action="">
            ...
            <div class="mb-4">
                <label for="modal-status" class="block text-sm font-semibold text-gray-700 mb-1">Estado de la Cita</label>
                <select name="status" id="modal-status" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-amber-500">
            ...
            <div class="mb-4">
                <label for="modal-payment" class="block text-sm font-semibold text-gray-700 mb-1">Método de Pago</label>
                <select name="payment_method" id="modal-payment" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-amber-500">
            ...
            <div class="mb-6">
                <label for="modal-tip" class="block text-sm font-semibold text-gray-700 mb-1">Propina ($)</label>
                <input type="number" step="0.50" min="0" name="tip" id="modal-tip" placeholder="0.00"
                    class="w-full border border-gray-300 rounded-lg p-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-amber-500">
            </div>
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit"
                    class="px-4 py-2 bg-gray-900 hover:bg-gray-800 text-white font-semibold rounded-lg text-sm transition">
                    Guardar Cambios
                </button>
            </div>
        </form>
